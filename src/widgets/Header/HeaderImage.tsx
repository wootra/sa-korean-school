...
const HeaderImage = ({ lang }: { lang: Languages }) => {
	...
	return (
		<>
			<Link href={`/${lang}/entry`} className='hidden sm:flex py-2 md:py-4 font-bold gap-2'>
				<Image
					...
					width='40'
				/>
				<span className='flex flex-col' style={{ fontSize: 'clamp(0.2rem, 3vw, 1.2rem)' }}>
			...
			>
				<Image
					...
					width='40'
				/>
				<span className='flex flex-col font-bold'>{multiLineTextIntoBlocks(navs.logo, 'text-nowrap')}</span>
